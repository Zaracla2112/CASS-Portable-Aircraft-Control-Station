                ...
                for intento in range(5):
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            print(f"[PAR] Descargado z{zoom}/{x}/{y}")
                        else:
                            print(f"[PAR] Fallo z{zoom}/{x}/{y}: {response.status_code}")
                        break
                    except requests.exceptions.RequestException as error:
                        espera = 2 ** intento
                        print(
                            f"[PAR] Error de red z{zoom}/{x}/{y}: {error}. "
                            f"Reintentando en {espera}s ({intento + 1}/5)"
                        )
                        time.sleep(espera)
                else:
                    print(f"[PAR] z{zoom}/{x}/{y} fallo despues de 5 intentos, se salta")
